                  ...
                  if (isset($_POST['btn'])) {
                    ...
                    if (preg_match($phoneRegex, $_POST['phone']) && filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
                      ...
                      if (checkdnsrr($domain, 'MX')) {
                          $req = "insert into utilisateurs (type ,prenom, nom, phone, email, adresse, login, psw)
                      values (2,'" . ucfirst($_POST['prenom']) . "', '" . strtoupper($_POST['nom']) . "', '" . $_POST['phone'] . "','" . $_POST['email'] . "','" . $_POST['niveau'] . "', '" . $_POST['login'] . "','" . $_POST['psw'] . "')";
                          $res = $pdo->query($req);
