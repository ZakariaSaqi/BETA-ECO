...
if (!isset($_SESSION['idd'])) {
    ...
    if (isset($_GET['action'])) {
        if ($_GET['action'] == 'view') {
            $id = $_GET['id'];
            $req = "select * from utilisateurs where type=2 and id_user=$id ";
            $res = $pdo->query($req);
    ...
    if ($_GET['action'] == 'delete') {
        $id = $_GET['id'];
        $req = "delete from utilisateurs where id_user=$id";
        $res = $pdo->query($req);
    }
}
?>
